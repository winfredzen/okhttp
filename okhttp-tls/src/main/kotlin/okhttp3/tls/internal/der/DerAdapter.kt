...
internal abstract class DerAdapter<T>(
   ...
   * ```
   *
   * You may also specify a tag class like [DerReader.TAG_CLASS_APPLICATION]. The default tag class
   * is [DerReader.TAG_CLASS_CONTEXT_SPECIFIC].
   *
   * ```
   * Point ::= SEQUENCE {
   ...
   * ```
   */
  fun withTag(tagClass: Int = DerReader.TAG_CLASS_CONTEXT_SPECIFIC, tag: Long): DerAdapter<T> {
    return copy(
        tagClass = tagClass,
        tag = tag
    )
  ...
  fun withExplicitBox(
    tagClass: Int = DerReader.TAG_CLASS_CONTEXT_SPECIFIC,
    tag: Long
  ): DerAdapter<T> {
